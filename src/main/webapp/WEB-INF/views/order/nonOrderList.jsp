...
<div class="container">
	...
	<h1 style="font-family: sans-serif; font-weight: bold; font-size: 35px">주문관리</h1>
		...
		<ul class="nav">
			...
			<c:choose>
				<c:when test="${!empty nonOrderList}">
				<table class="table table-hover">
					...
					<c:if test="${member.mlevel == 0}">
						<c:forEach items="${nonOrderList}" var="nonOrderList">
							<tr>
								<td>${nonOrderList.order_date}</td>
								<td><p><a href="/order/orderView?order_code=${nonOrderList.order_code}">${nonOrderList.order_code}</a></p></td>
								<td>non-member[비회원]</td>
								<td>${nonOrderList.product_name}</td>
								<td>${nonOrderList.order_count}</td>
								<td><fmt:formatNumber pattern="###,###,###" value="${nonOrderList.total_price}"/>원</td>
								<td>${nonOrderList.order_status}</td>
							</tr>
						</c:forEach>	
					</c:if>
					</tbody>
